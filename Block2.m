...
numlines = 1;
defaultanswer = {'XXX', '100', '0.5', '16', '0.85', '0.6', '0'};
answers = inputdlg(prompts, name, numlines, defaultanswer);

fb = str2double(answers{7}); % Feedback. 1 for True, 0 for False
subjectName = answers{1};

currentDateTime = datestr(now, 'yyyymmdd_HHMMSS');

fileName = sprintf('%s_Block2_%s.csv', subjectName, currentDateTime);
...
try
    ...
    initialContrast = str2double(answers{5}); % Initial contrast value for target grating
    peripheralContrast = str2double(answers{6}); % Contrast for peripheral gratings
    ...
    for trial = 1:numTrials
        ...
        peripheralGabor = cos((2 * pi * spatialFrequency) * x) .* exp(-((x).^2 + (y).^2) / (sigma^2)); % 0.4 must be asked from researcher
